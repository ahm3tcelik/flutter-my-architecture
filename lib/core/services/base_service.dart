...
class BaseService<T extends IEntity, TLocalDataSource extends ILocalDataSource<T>> implements IService<T> {
  ...
  Future<Either<Failure, int>> localAdd(T ent) async {
    ...
    if (newId == null) {
      return const Left<Failure, int>(Failure("Data could not be added"));
    }
    return Right<Failure, int>(newId);
  }
  ...
  Future<Either<Failure, int>> localDeleteById(dynamic id, T ent) async {
    ...
    if (affectedNum == null || affectedNum < 1) {
      return const Left<Failure, int>(Failure("Data could not be deleted"));
    }
    return Right<Failure, int>(affectedNum);
  }
  ...
  Future<Either<Failure, T>> localGetById(dynamic id) async {
    ...
    if (data == null) {
      return Left<Failure, T>(Failure('No data available'));
    }
    return Right(data);
  }
  ...
  Future<Either<Failure, List<T>>> localGetAll() async {
    ...
    if (data == null || data.isEmpty) {
      return Left<Failure, List<T>>(Failure('No data available'));
    }
    return Right(data);
  }
  ...
  Future<Either<Failure, int>> localUpdateById(dynamic id, T ent) async {
    ...
    if (affectedNum == null || affectedNum < 1) {
      return const Left<Failure, int>(Failure("Data could not be updated"));
    }
